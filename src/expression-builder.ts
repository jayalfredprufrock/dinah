...
export class ExpressionBuilder {
	...
	get attributeNames(): Obj<string> | undefined {
		return this.attrNames.size
			? Object.fromEntries([...this.attrNames.entries()].map(([name, placeholder]) => [placeholder, name]))
			: undefined;
	}

	get attributeValues(): Obj | undefined {
		return this.attrValues.size
			? Object.fromEntries([...this.attrValues.entries()].map(([value, placeholder]) => [placeholder, value]))
			: undefined;
	}
	...
	condition<T extends Obj | undefined>(expression: T): T extends undefined ? undefined : string {
		...
		for (const [key, val] of Object.entries(expression)) {
			if (key === '$and') {
				...
				throw new Error(`Unexpected operator "${key}". Expected attribute path or compound operator.`);
			}
		}
	...
	protected resolveCondition(path: string, exp: unknown): string {
		...
		switch (operator) {
			...
			case '$size': {
				...
				if (!sizeExp || typeof sizeExp !== 'object' || !Object.keys(sizeExp).at(0)?.startsWith('$')) {
	...
	protected resolveSetOperand(path: string, operand: unknown): string {
		if (isOperation(operand)) {
			...
			if (operand.$append !== undefined || operand.$prepend !== undefined) {
				...
				const params = operand.$append ? [{ $path: path }, resolvedListOperand] : [resolvedListOperand, { $path: path }];
