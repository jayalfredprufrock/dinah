...
```typescript
// total = subtotal + tax
{ total: { $plus: [{ $path: "subtotal" }, { $path: "tax" }] } }
```
...
{ createdAt: { $ifNotExists: Date.now() } }
```
...
{ createdAt: { $ifNotExists: ["firstSeenAt", Date.now()] } }
```
...
```typescript
{ tags: { $setAdd: "featured" } }       // ADD tags {"featured"}
{ tags: { $setAdd: ["a", "b", "c"] } }  // ADD tags {"a","b","c"}
{ tags: { $setDel: "obsolete" } }       // DELETE tags {"obsolete"}
```

Pass a JavaScript `Set` to avoid the array-wrapping step:

```typescript
{ tags: { $setAdd: new Set(["a", "b"]) } }
```
